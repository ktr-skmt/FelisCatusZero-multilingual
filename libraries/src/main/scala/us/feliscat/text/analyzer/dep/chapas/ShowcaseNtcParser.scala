...
import us.feliscat.text.analyzer.dep.{Chunk, Chunks, DepType}
...
object ShowcaseNtcParser extends NtcParser {
  def parse(ntcText: Iterator[String]): Doc = {
    ...
    var doc: Doc = null
    var sentences = ArrayBuffer.empty[Chunks]
    var chunks = ArrayBuffer.empty[Chunk]
    var tokens = ArrayBuffer.empty[Token]
