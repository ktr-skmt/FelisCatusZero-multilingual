...
import us.feliscat.util.{ChaPASCache, SentenceUnitReconstructorConfig}
...
object ChaPAS extends PasAnalyzer with EchoProcess with ChaPASCache {
  ...
  def analyze(us.feliscat.sentence: NormalizedStringOption): Stream[String] = {
    (echo(us.feliscat.sentence) #| analyzer()).lines_!
  }
  private def analyzer(): ProcessBuilder = {
    Process(Seq("java", "-Xmx1g", "-jar", cmd.toAbsolutePath.toString, "-I", "RAW"))
  }
  private def echo(us.feliscat.sentence: NormalizedStringOption): ProcessBuilder = {
    us.feliscat.sentence match {
...
*/
  ...
  override final protected[this] def textToDoc(normalizedSentence: JapaneseNormalizedString): Doc = {
    (if (cacheEnable) {
    ...
    analyze(NormalizedStringOption(NormalizedString(StringOption(
      ...
      line =>
        ...
        if (line startsWith "*") {
          ...
          if (tokens.length == 4) {
            morphemeBuffer += line
            val us.feliscat.text = tokens.head
            //val pos  = tokens(1)
            //val bio  = tokens(2)
            ...
            bunsetsuBuffer.append(us.feliscat.text)
          }
        }
        */
//    }
//  }
  ...
  public class ChaPASOptions {
    ...
    @Option(
        name = "--print-us.feliscat.sentence-id",
        usage = "print us.feliscat.sentence id"
    )
