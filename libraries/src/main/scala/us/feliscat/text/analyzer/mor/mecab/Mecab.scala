...
abstract class Mecab extends MorphemeAnalyzer {
  override def analyzer(): ProcessBuilder = {
    Process("mecab" :: "-d" :: LibrariesConfig.mecabUnidicDir :: Nil)
  }
  ...
  override def extractMorphemes(sentence: StringOption): Seq[OriginalTextTextPOS] = {
    val buffer = ListBuffer.empty[(String, String, String, StringOption, Seq[String])]
      ...
      case line if line != "EOS" =>
        ...
        if (tokens.length == 2) {
          ...
          if (textColumn < subTokens.length) {
            val textOpt = StringOption(subTokens(textColumn).trim)
            val posOpt = StringOption(subTokens.take(textColumn).mkString(",").trim)
