...
import us.feliscat.converter.KanjiNumeralParser
...
object IpadicMecab extends Mecab {
  override def analyzer(): ProcessBuilder = {
    val commands: Seq[String] = LibrariesConfig.mecabIPADicUserDic match {
      case StringSome(userDic) =>
        "mecab" :: "-d" :: LibrariesConfig.mecabIPADicDir :: "-u" :: userDic :: Nil
      case StringNone =>
        "mecab" :: "-d" :: LibrariesConfig.mecabIPADicDir :: Nil
    }
    Process(commands)
  }
  ...
  override protected def extractWords(sentence: StringOption, needsContentWords: Boolean): Seq[String] = {
    ...
    val compoundNounBuilder   = new StringBuilder(0)
    val compoundAdverbBuilder = new StringBuilder(0)
    val compoundVerbBuilder   = new StringBuilder(0)
    val numerals              = new StringBuilder(0)
