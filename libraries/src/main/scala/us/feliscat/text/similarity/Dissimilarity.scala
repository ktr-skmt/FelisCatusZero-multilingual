...
import us.feliscat.text.vector.FrequencyVector
...
object Dissimilarity extends Enumeration {
  ...
  def calculate(v1: FrequencyVector, v2: FrequencyVector, dissimilarity: Dissimilarity.Value): Double = {
    ...
    dissimilarity match {
      ...
      case Minkowsky =>
        calculateMinkowsky(v1, v2, LibrariesConfig.minkowskyQ)
      case _ =>
  ...
  def calculateChebyshev(v1: FrequencyVector, v2: FrequencyVector): Double = {
    var distance: Int = 0
    val vector1: mutable.Map[String, Int] = v1.vector
    val vector2: mutable.Map[String, Int] = v2.vector
    vector1 foreach {
      case (key1, value1) =>
