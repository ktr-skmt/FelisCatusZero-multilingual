...
/**
  ...
  */
object JaroWinklerDistance extends JaroWinklerDistance(LibrariesConfig.jaroWinklerThreshold, LibrariesConfig.jaroWinklerScalingFactor)

/**
  * @author K.Sakamoto
  */
object JaroDistance extends JaroWinklerDistance(LibrariesConfig.jaroWinklerThreshold, LibrariesConfig.jaroWinklerScalingFactor) {
...
class JaroWinklerDistance(threshold: Double, scalingFactor: Double) extends Distance {
  ...
  def calculateJaroDistance[Element](array1: Array[Element], array2: Array[Element]): Double = {
    ...
    def matches(array1: Array[Element], array2: Array[Element]): (Int, Int, Int, Int) = {
      ...
      val ms1 = new Array[PseudoElement](matches)
      val ms2 = new Array[PseudoElement](matches)
      var si: Int = 0
      for (i <- min.indices) {
        if (matchIndexes(i) != -1) {
