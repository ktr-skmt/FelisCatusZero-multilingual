package us.feliscat.text.kanji
...
trait KanjiCharacter {
  ...
  lazy val regex: String = {
    val builder = new StringBuilder(kanji.size)
  ...
  protected def readKanjiCSV(fileName: String): Seq[String] = {
    ...
    val file: File = Paths.get(LibrariesConfig.resourcesDir, "kanji", fileName.concat(".csv")).toAbsolutePath.toFile
