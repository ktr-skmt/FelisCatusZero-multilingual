package us.feliscat.exam.national_center_test.xml.choice

import us.feliscat.exam.national_center_test.xml.choice.collcetion.{ChoiceCollection, ChoiceSet, ChoiceSingleton}
...
object ChoiceParser {
  def parse(choices: NodeSeq): ChoicesImpl = {
    val buffer = ListBuffer.empty[ChoiceCollection]

    val defaultChoice = new ChoicesBinary(Nil)
    ...
    def isChoiceSet(choiceText: String): Boolean = {
      ...
      val pattern: Regex = s"($choiceSetElementPattern)+".r
    ...
    choices \ "choice" foreach {
      choice =>
        ...
        val choiceRemovedCNum: String = JapaneseNormalizer.
        ...
        buffer += {
          if (isChoiceSet(choiceRemovedCNum)) {
            val map = mutable.Map.empty[String, ChoiceSingleton]
