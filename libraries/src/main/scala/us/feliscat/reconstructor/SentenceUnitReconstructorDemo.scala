...
import us.feliscat.reconstructor.unit.{CombineUnit, DirectDependentsUnit, PAUnit}
import us.feliscat.sentence.ja.JapaneseSentenceSplitter
import us.feliscat.text.StringOption
import us.feliscat.text.analyzer.dep.chapas.{ChaPAS, Doc, PasAnalyzer, Showcase}
import us.feliscat.text.analyzer.dep.{Chunks, DepType}
import us.feliscat.text.normalizer.ja.JapaneseNormalizedString
import us.feliscat.util.{Color, FileOps, SentenceUnitReconstructorConfig}
...
object SentenceUnitReconstructorDemo {
  def main(args: Array[String]): Unit = {
    ...
    FileOps.write(outputStream) { writer =>
      ...
      dDependentsUnitsBySentence zip pasUnitsBySentence foreach { case (dDependentsUnits, paUnits) =>
        ...
        // CaboCha output
        val normalizedSentence: JapaneseNormalizedString =
          JapaneseNormalizedString(StringOption(sentence.text))
  ...
  final private[this] def parseArg(args: Array[String]): CommandLine = {
    ...
    options.addOption("color", false, "Coloring the output us.feliscat.text for console.")
