...
import us.feliscat.time.{MultiLingualTimeExtractorInTimeExpression, TimeTmp}
...
/**
  ...
  * @author K.Sakamoto
  */
object JapaneseTimeExtractorInTimeExpression extends MultiLingualTimeExtractorInTimeExpression with Japanese {
  ...
  */
  ...
  override def extract(sentence: StringOption): Seq[TimeTmp] = {
    ...
    {for (m <- timeRegex.findAllMatchIn(
      JapaneseTimeExtractionPreprocessor.convert(sentence).getOrElse("")
    ) if isTime(m)) yield {
