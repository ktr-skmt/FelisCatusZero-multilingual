...
import us.feliscat.util.GzipXMI
...
class GzipXmiCasConsumer extends CasConsumer_ImplBase with GzipXMI {
  ...
  private var mOutputDir = Option.empty[File]
  ...
  override def processCas(aCAS: CAS): Unit = {
    ...
    var aJCasOption = Option.empty[JCas]
    try {
  ...
  private def writerXmi(aCAS: CAS, outputFile: File): Unit = {
    var out     = Option.empty[FileOutputStream]
    var gZipOut = Option.empty[GZIPOutputStream]
    ...
    try {
      ...
      val ser    = new XmiCasSerializer(aCAS.getTypeSystem)
      val xmlSer = new XMLSerializer(o, false)
