...
import uima.cc.en.EnglishEssayEvaluator
import uima.cc.ja.JapaneseEssayEvaluator
import util.Config
...
/**
  ...
  *         Created on 15/10/30
  */
class EssayEvaluator extends CasConsumer_ImplBase with JapaneseDocumentAnnotator {
  ...
  override def processCas(aCAS: CAS): Unit = {
    ...
    JapaneseEssayEvaluator.process(aCAS) match {
    ...
    EnglishEssayEvaluator.process(aCAS) match {
      case StringSome(result) =>
        builder.append(result)
      case StringNone =>
        // Do nothing
