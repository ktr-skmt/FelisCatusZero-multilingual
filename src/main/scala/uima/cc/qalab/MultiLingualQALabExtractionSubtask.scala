...
import us.feliscat.text.similarity.OverlapCalculator
import us.feliscat.text.vector.BinaryVectorGeneratorFromJCas
import us.feliscat.types.{Question, TextAnnotation, Time}
...
trait MultiLingualQALabExtractionSubtask extends MultiLingualQALabSubmission {
  ...
  protected def scoreForText(question: TextAnnotation, document: TextAnnotation): Double = {
    OverlapCalculator.calculate(
      BinaryVectorGeneratorFromJCas.getVectorFromAnnotation(question),
      BinaryVectorGeneratorFromJCas.getVectorFromAnnotation(document)
    )
  }
