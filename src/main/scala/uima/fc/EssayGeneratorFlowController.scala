...
/**
  ...
  * @author K.Sakamoto
  */
object EssayGeneratorFlowController {
  ...
  def printAnalysisEngines(): Unit = {
    println("Essay Generator Flow Controller Analysis Engine List:")
...
class EssayGeneratorFlowController extends CasFlowController_ImplBase  {
  ...
  class AnalyzerFlow extends CasFlow_ImplBase {
    ...
    override def next(): Step = {
      println(">> Essay Generator Flow Controller Processing")
      val aCAS: CAS = getCas
    ...
    private def inputsSatisfied(aeKey: String, aInputs: Array[TypeOrFeature], aCAS: CAS): Boolean = {
      if (step < 0 || EssayGeneratorFlowController.analysisEngineList.length <= step) {
        return false
      }
      if (aeKey == EssayGeneratorFlowController.analysisEngineList(step)) {
        return true
      }
      false
    }
