...
class GzipXmiCollectionReader extends CollectionReader_ImplBase with GzipXMI {
  ...
  override def initialize(): Unit = {
    ...
    val directory = new File(getConfigParameterValue(PARAM_INPUT_DIR).asInstanceOf[String].trim)
  ...
  override def getNext(aCAS: CAS): Unit = {
    ...
    val input = Option[FileInputStream](new FileInputStream(currentFile))
    if (input.isEmpty) {
      return
    }
    val inputStream: FileInputStream = input.get

    val gZipInput = Option[GZIPInputStream](new GZIPInputStream(inputStream))
