...
/**
  ...
  * @author K.Sakamoto
  */
trait Corrector extends MultiLingualDocumentAnnotator {
  protected val mYamakawaWorldHistoryGlossaryDocnoPrefix: String = "YamakawaWorldHistoryGlossary"

  protected def extractTimeFromPreviousParagraph(docnoOpt: StringOption): TimeTmp
  protected def normalizeSentence(str: StringOption): StringOption

  protected def correct(text: String, keywordOriginalTextOpt: StringOption, expansionOnlyList: Seq[String]): String
  ...
  def correct(aJCas: JCas,
    ...
    //passageText, passage
    val passageMap = mutable.Map.empty[String, Passage]

    val documentBuffer = ListBuffer.empty[Document]

    keyword.getDocumentSet.toArray foreach {
      documentTmp =>
        val document: Document = documentTmp.asInstanceOf[Document]
        val keywordBuffer = ListBuffer.empty[String]
        ...
        keywordCorrectionMap foreach {
          case (k, expansions) =>
            ...
            val text: String = correct(document.getText, StringOption(k), expansionList)
        ...
        if (hasTimeRange(Option(initialSentence4Doc.getBeginTime), Option(initialSentence4Doc.getEndTime))) {
          ...
          val timeInDocTitle: TimeTmp = extractTime(StringOption(document.getTitle))
          if (timeInDocTitle.beginTime.nonEmpty || timeInDocTitle.endTime.nonEmpty) {
            ...
            docnoOpt match {
              case StringSome(docno) if !docno.startsWith(mYamakawaWorldHistoryGlossaryDocnoPrefix) =>
                val timeTmp: TimeTmp = extractTimeFromPreviousParagraph(docnoOpt)
        ...
        if (!(
          ...
          sentenceList foreach {
            sentence: Sentence =>
              val sentenceText: String = normalizeSentence(StringOption(sentence.getOriginalText)).get
              sentence.setText {
