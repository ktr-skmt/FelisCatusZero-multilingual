package modules.exam.national_center_test.xml

import java.io.{BufferedReader, File}
import java.nio.file.Paths

import modules.util.ModulesConfig
import us.feliscat.exam.national_center_test.xml.{BigQuestion, MiddleQuestion, NationalCenterTestQuestion, SmallQuestion}
import us.feliscat.exam.national_center_test.xml.choice.ChoiceParser
import us.feliscat.text.normalizer.ja.JapaneseNormalizer
import us.feliscat.text.{StringNone, StringOption, StringSome}
import us.feliscat.util.{Logger, StaticValue}
...
class NationalCenterTestParser {
  ...
  private def extractBigQuestions(exam: NodeSeq): Seq[BigQuestion] = {
    for (bigQuestion <- exam \ "uima.modules.qa.question") yield {
      new BigQuestion(
  ...
  private def extractMiddleQuestions(bigQuestion: NodeSeq): Seq[MiddleQuestion] = {
    val middleQuestionBuffer = ListBuffer.empty[MiddleQuestion]
    val smallQuestionBuffer = ListBuffer.empty[SmallQuestion]
    ...
    def underLinePart(smallQuestion: NodeSeq): StringOption = {
      Option(smallQuestion \ "instruction" \ "ref" \ "@target") match {
        case Some(target) =>
          ...
          var ret = StringOption.empty
          targetUText foreach {
            uText =>
              uText.child foreach {
                case u if u.label != "label" =>
                  ret = JapaneseNormalizer.normalize(StringOption(u.text.trim))
                case _ =>
                  // Do nothing
              }
          }
        ...
        lText =>
          lText.child foreach {
            l =>
              if (l.label == "label") {
                val label: NodeSeq = l
                key = JapaneseNormalizer.normalize(StringOption(label.text.trim)).getOrElse("")
              } else if (l.label == "br") {
                //no process
              } else {
                val text = StringOption(l.text.trim)
                if (text.nonEmpty) {
                  JapaneseNormalizer.normalize(text) match {
          ...
          middleQuestionElement =>
            if (middleQuestionElement.label == "data") {
              ...
              context = data.child filter(
                element =>
                  element.label != "br")
            } else if (middleQuestionElement.label == "uima.modules.qa.question") {
  ...
  private def centerTestReader(fileName: String): Option[BufferedReader] = {
    val path = Paths.get(
      ModulesConfig.centerTestJaXmlDir,
      File.separator,
      fileName
    )
