...
object QALabDataset2MultiLingualQACorpusConverter {
  ...
  def convert(): Unit = {
    ...
    val xmlSchema: XmlSchema = ModulesConfig.qalabDatasetXmlSchema
    Paths.get(ModulesConfig.qalabDatasetPath).toFile.listFiles foreach {
      case file: File
        if file.canRead &&
           file.isFile &&
           file.getName.endsWith(".xml") &&
           xmlSchema.isValid(new StreamSource(file)) =>
