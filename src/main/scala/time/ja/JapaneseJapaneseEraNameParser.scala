...
/**
  ...
  * @author K.Sakamoto
  */
object JapaneseJapaneseEraNameParser extends MultiLingualJapaneseEraNameParser with Japanese {
  ...
  override def convertToRomanCalendar(text: StringOption): StringOption = {
    text map {
      str: String =>
        var t: String = str
        ...
        while (mOpt.nonEmpty) {
          ...
          val regnalYear = StringOption(m.group(1))
