...
class Skip2GramListBuffer {
  private val map = mutable.Map.empty[String, mutable.Map[String, mutable.Map[Int, Int]]]
  ...
  def toMap: Map[String, Map[String, Map[Int, Int]]] = {
    val mapTemp = mutable.Map.empty[String, Map[String, Map[Int, Int]]]
    map foreach {
      case (firstGram, map1) =>
        val map1Temp = mutable.Map.empty[String, Map[Int, Int]]
        map1 foreach {
          case (lastGram, map2) =>
  ...
  def result: Skip2GramList = {
    val buffer = ListBuffer.empty[Skip2Gram]
    toMap foreach {
      case (firstGram, map1) =>
        map1 foreach {
          case (lastGram, map2) =>
