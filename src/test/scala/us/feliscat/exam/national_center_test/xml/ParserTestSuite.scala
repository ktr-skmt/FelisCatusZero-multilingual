package us.feliscat.exam.national_center_test.xml

/*
import us.feliscat.exam.QuestionFormat
import us.feliscat.exam.national_center_test.xml.model.CSVQuestionFormat
...
class ParserTestSuite() extends AssertionsForJUnit() {
  @Theory def testQuestionFormat(year : Int): Unit = {
    val formats = mutable.HashMap.empty[String, CSVQuestionFormat]
    ...
    for (line <- file.getLines) {
      ...
      val qf = new CSVQuestionFormat(cols(1), cols(2))

      formats.put(cols.head, qf)
    }

    val parser = new NationalCenterTestParser()
